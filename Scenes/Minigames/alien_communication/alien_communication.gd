...
var trie
var found_words = []
var words_needed = 3 - CurGameState.total_time_bonus
...
func create_ui():
	...
	info_label.text = "Find " + str(words_needed) + " words!"
...
func _finish(status: String) -> void:
	...
	emit_signal("done", {"status": status, "score": found_words.size()})
	queue_free()

func start_timer() -> void:
	add_child(timer)
	var time_bonus = CurGameState.total_time_bonus

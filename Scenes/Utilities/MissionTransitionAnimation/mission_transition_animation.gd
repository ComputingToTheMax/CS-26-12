...
func play_animation():
	...
	current_tween.tween_property(animation_camera, "offset", holding_offset, animation_duration * 2 / 3).set_ease(Tween.EASE_IN_OUT)
	current_tween.tween_callback(_end_animation)
	
func _end_animation():
	holding_timer.start()
	await holding_timer.timeout
	create_tween().tween_property(animation_camera, "offset", target_offset, animation_duration / 3.).set_ease(Tween.EASE_IN_OUT)
